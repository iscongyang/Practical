...
function set_supervisor() {
    # 放置Java程序
    mv /root/park/a /app/mscommon
    # 设置supervisor的conf
	echo "files = supervisord.d/*.conf" >> /etc/supervisord.conf
	cat > /etc/supervisord.d/mscommon.conf << EOF
[program:mscommon]
command=/usr/bin/java -XmsxM -XmxxM  -XX:+HeapDumpOnOutOfMemoryError -XX:+PrintGCDetails -Xloggc:./logs/gc.log -XX:+PrintGCTimeStamps -jar xxx.jar
...
function set_docker() {
    ...
    docker load < /root/park/x.tar
    docker run -d --network=host --restart always -v /app/logs:/app/charge_tool/logs --env mysql_host=127.0.0.1 --env mysql_pwd='xxx'  xx_images
