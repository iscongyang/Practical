...
logging.basicConfig(filename='open_vpn_tell.log', level=logging.INFO, format='[%(asctime)s] %(filename)s[line:%(lineno)d] %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
def get_up():
    ...
    name_mail = input("请输入要发送的邮箱(默认bangdao-tech.com,不使用此邮箱请完整输入): ")
    list_mail = list(name_mail)
    if "@" in list_mail:
        name_mail = name_mail
    else:
        name_mail = name_mail + "@bangdao-tech.com"

    new_user = "bd" + name_id
    user_pwd = "Bangdao" + name_id
...
def send_mail2(obj):
    ...
    mail_user = "zabbix@xxx.com"  # 邮箱用户名
    mail_pass = "xxx"  # 授权码

    sender = 'zabbix@xxx.com'  # 发件人

    receivers = []  # 接收人列表
    receivers.append(obj[2])

    context = "您好: \n \n\n您的VPN账号已开通, 可使用该账号登录服务器\n" \
    "服务器地址为: x.x.x.x, 端口为xx\n" \
